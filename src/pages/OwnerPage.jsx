...
function Badge({ status }) {
  const map = { hadir: [C.okBg, C.ok, 'Hadir'], sakit: [C.infBg, C.inf, 'Sakit'], cuti: [C.okBg, C.ok, 'Cuti'], ctb: [C.purBg, C.pur, 'CTB'], day_off: ['#E8F5E9', '#2E7D32', 'Day Off'] }
...
function LogTab({ employees, today }) {
  ...
  async function fetchLogs(date) {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('attendance')
        .select('employee_id, date, check_in, check_out, status, is_late, late_minutes, gps_dist_in, doc_url, employees!employee_id(name, role)')
        .eq('date', date)
  ...
  const stats = { hadir: 0, sakit: 0, cuti: 0, ctb: 0, day_off: 0 }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 6 }}>
        {[['Hadir', stats.hadir, C.ok], ['Sakit', stats.sakit, C.inf], ['Cuti', stats.cuti, C.ok], ['Day Off', stats.day_off, '#2E7D32'], ['CTB', stats.ctb, C.pur]].map(([l, v, c]) => (
...
function ReportTab({ employees }) {
  ...
  async function fetchReport() {
    ...
    const end = month + '-31'
    const { data: rows } = await supabase.from('attendance').select('*, employees!employee_id(name,role,leave_balance)').gte('date', start).lte('date', end)
    // Group by employee
    const grouped = {}
    ;(rows || []).forEach(r => {
