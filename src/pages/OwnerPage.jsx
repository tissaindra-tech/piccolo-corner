...
function calcGajiBulanan(emp,atts,potongans,bonuses){
  ...
  return {pokok,tunj_total,bonus_total,insentif,potongan_total,total_pendapatan,total_potongan,gaji_bersih}
}
...
function ReportTab({employees}){
  ...
  function exportExcel(){
    ...
    if(view==='gaji'||filterEmpId!=='all'){
      const ws4=[
        ...
        [],
        ['Nama','Jabatan','Gaji Pokok','Tunjangan Tetap','Bonus Tidak Tetap','Insentif','Total Pendapatan','Potongan','Gaji Bersih']
      ]
      empData.forEach(d=>{
        const empPotongs=potongans.filter(p=>p.employee_id===d.id)
        const empBonuses=bonuses.filter(b=>b.employee_id===d.id)
        const calc=calcGajiBulanan({gaji_pokok:d.gaji_pokok,tunjangan:d.tunjangan,incentive_enabled:d.incentive_enabled},d._atts,empPotongs,empBonuses)
        const bonusNames=empBonuses.length>0?empBonuses.map(b=>b.nama+'(Rp '+parseInt(b.nominal).toLocaleString('id-ID')+')').join(', '):'—'
        ws4.push([d.name,d.role,'Rp '+calc.pokok.toLocaleString('id-ID'),'Rp '+calc.tunj_total.toLocaleString('id-ID'),'Rp '+calc.bonus_total.toLocaleString('id-ID')+' | '+bonusNames,(calc.insentif>=0?'Rp ':'-Rp ')+Math.abs(calc.insentif).toLocaleString('id-ID'),'Rp '+calc.total_pendapatan.toLocaleString('id-ID'),'Rp '+calc.total_potongan.toLocaleString('id-ID'),'Rp '+calc.gaji_bersih.toLocaleString('id-ID')])
      })
  ...
  async function tandaiBayar(){
    ...
    const calc=calcGajiBulanan(emp,atts,pots,bons)
      ...
      {gajiModal&&(
        <div onClick={()=>setGajiModal(null)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.6)',zIndex:50,display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
          <div onClick={e=>e.stopPropagation()} style={{background:T.surface,borderRadius:'18px 18px 0 0',width:'100%',maxWidth:520,maxHeight:'90vh',overflowY:'auto',padding:'16px 14px 80px'}}>
            ...
            {(() => {
              const calc=calcGajiBulanan(gajiModal.emp,gajiModal.atts,gajiModal.potongans,gajiModal.bonuses||[])
