// OwnerPage v3.0 - + Daily Worker (Magang & Full Day), gaji dari absensi
...
function calcGajiBulanan(emp,atts,potongans,bonuses,rates){
  ...
  // ----- KARYAWAN TETAP / PROBATION (logika lama, tidak berubah) -----
  ...
  const total_pendapatan=pokok+tunj_total+bonus_total+(insentif>0?insentif:0)
  const total_potongan=potongan_total+(insentif<0?Math.abs(insentif):0)
  const gaji_bersih=total_pendapatan-total_potongan
  return {tipe,pokok,tunj_total,bonus_total,insentif,potongan_total,total_pendapatan,total_potongan,gaji_bersih,dw_total:0,dw_warn:0,dw_hari:0}
}
...
function EmployeesTab({employees,onRefresh}){
  const [form,setForm]=useState({name:'',role:'',phone:'',pin:'',shift:'',leave_balance:12,gaji_pokok:0,tunjangan:[],tipe_karyawan:'tetap'})
  ...
  async function addEmp(){
    ...
    setSaving(false);setAdding(false);setForm({name:'',role:'',phone:'',pin:'',shift:'',leave_balance:12,gaji_pokok:0,tunjangan:[],tipe_karyawan:'tetap'});onRefresh()
  }
  function startEdit(emp){setEditingId(emp.id);setEditForm({name:emp.name,role:emp.role,phone:emp.phone,pin:'',shift:emp.shift||'',leave_balance:emp.leave_balance,incentive_enabled:emp.incentive_enabled!==false,gaji_pokok:emp.gaji_pokok||0,tunjangan:emp.tunjangan||[],tipe_karyawan:emp.tipe_karyawan||'tetap'})}
  async function saveEdit(id){
    setSaving(true)
    const isDW=editForm.tipe_karyawan==='dw_magang'||editForm.tipe_karyawan==='dw_fullday'
    const u={name:editForm.name,role:editForm.role,phone:editForm.phone,shift:editForm.shift,leave_balance:parseInt(editForm.leave_balance),tipe_karyawan:editForm.tipe_karyawan||'tetap',incentive_enabled:isDW?false:editForm.incentive_enabled,gaji_pokok:isDW?0:(parseInt(editForm.gaji_pokok)||0),tunjangan:isDW?[]:(editForm.tunjangan||[])}
...
function ReportTab({employees}){
  ...
  function exportExcel(){
    ...
    if(view==='gaji'||filterEmpId!=='all'){
      const ws4=[
        ...
        [],
        ['Nama','Jabatan','Tipe','Gaji Pokok / Upah','Tunjangan Tetap','Bonus Tidak Tetap','Insentif','Total Pendapatan','Potongan','Gaji Bersih']
      ]
      empData.forEach(d=>{
        const empPotongs=potongans.filter(p=>p.employee_id===d.id)
        const empBonuses=bonuses.filter(b=>b.employee_id===d.id)
        const calc=calcGajiBulanan({gaji_pokok:d.gaji_pokok,tunjangan:d.tunjangan,incentive_enabled:d.incentive_enabled,tipe_karyawan:d.tipe_karyawan},d._atts,empPotongs,empBonuses,dwRates)
        ...
        ws4.push([d.name,d.role,TIPE_LABEL[d.tipe_karyawan||'tetap'],'Rp '+calc.pokok.toLocaleString('id-ID'),'Rp '+calc.tunj_total.toLocaleString('id-ID'),'Rp '+calc.bonus_total.toLocaleString('id-ID')+' | '+bonusNames,isDW?'—':(calc.insentif>=0?'Rp ':'-Rp ')+Math.abs(calc.insentif).toLocaleString('id-ID'),'Rp '+calc.total_pendapatan.toLocaleString('id-ID'),'Rp '+calc.total_potongan.toLocaleString('id-ID'),'Rp '+calc.gaji_bersih.toLocaleString('id-ID')])
      })
      const sht4=XLSX.utils.aoa_to_sheet(ws4)
      sht4['!cols']=[{wch:22},{wch:14},{wch:20},{wch:16},{wch:14},{wch:24},{wch:14},{wch:16},{wch:14},{wch:16}]
  ...
  async function tandaiBayar(){
    ...
    const calc=calcGajiBulanan(emp,atts,pots,bons,dwRates)
  ...
  async function openGajiModal(emp){
    ...
    setGajiModal({emp,atts:atts||[],potongans:pots||[],bonuses:bons||[],bulan,label:getDateRange().label})
  }
                ...
                <div key={d.id} onClick={()=>openGajiModal(d)} style={{background:T.surface,borderRadius:12,border:`.5px solid ${isPaid?T.greenBd:T.border}`,padding:'11px 13px',cursor:'pointer'}}>
                  ...
                  {hasGaji&&(
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr 1fr',gap:6,padding:'8px 0 0',borderTop:`.5px dashed ${T.border}`}}>
                      <div><div style={{fontSize:8,color:T.muted,letterSpacing:'.04em'}}>{isDW?'UPAH HARIAN':'POKOK+TUNJ.'}</div><div style={{fontSize:11,fontWeight:700,color:T.black}}>Rp {(calc.pokok+calc.tunj_total).toLocaleString('id-ID')}</div></div>
      ...
      {gajiModal&&(
        <div onClick={()=>setGajiModal(null)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.6)',zIndex:50,display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
          <div onClick={e=>e.stopPropagation()} style={{background:T.surface,borderRadius:'18px 18px 0 0',width:'100%',maxWidth:520,maxHeight:'90vh',overflowY:'auto',padding:'16px 14px 80px'}}>
            ...
            {(() => {
              const calc=calcGajiBulanan(gajiModal.emp,gajiModal.atts,gajiModal.potongans,gajiModal.bonuses||[],dwRates)
