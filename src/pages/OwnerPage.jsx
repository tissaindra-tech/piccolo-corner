...
function ReportTab({employees}){
  ...
  async function tandaiBayar(){
    ...
    if(!window.confirm(`Tandai gaji ${emp.name} bulan ${label} sudah dibayarkan?\n\nGaji bersih: Rp ${calc.gaji_bersih.toLocaleString('id-ID')}\n\nSetelah ini, slip gaji akan terlihat oleh karyawan dan tidak bisa diubah.`)) return
    setPayingId(emp.id)
    // Buat snapshot lengkap
    const snapshot={
      ...
      dibayarkan_at:new Date().toISOString()
    }
    ...
    // Update modal state
    const pkey=emp.id+'_'+bulan
    setPaidSlips(prev=>({...prev,[pkey]:{employee_id:emp.id,bulan,snapshot,gaji_bersih:calc.gaji_bersih,dibayarkan_at:new Date().toISOString()}}))
              ...
              return(
                <>
                  ...
                  {(()=>{
                    ...
                    const paidAtModal=isPaidModal?new Date(paidSlips[pkey].dibayarkan_at).toLocaleDateString('id-ID',{day:'numeric',month:'long',year:'numeric'}):null
                    return isPaidModal?(
                      <>
                        <div style={{background:T.greenLight,border:`.5px solid ${T.greenBd}`,borderRadius:10,padding:'10px 14px',marginBottom:10,display:'flex',alignItems:'center',gap:8}}>
                          ...
                          <div style={{flex:1}}>
                            ...
                            <div style={{fontSize:10,color:T.green}}>{paidAtModal} · Slip gaji sudah terlihat oleh karyawan</div>
                          </div>
                        </div>
                        <div style={{display:'flex',gap:7}}>
                      ...
                      <div style={{display:'flex',gap:7}}>
                        ...
                        <button onClick={tandaiBayar} disabled={payingId===gajiModal.emp.id} style={{flex:2,padding:11,background:T.green,color:'#fff',border:'none',borderRadius:9,fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit',opacity:payingId===gajiModal.emp.id?.6:1}}>
                          {payingId===gajiModal.emp.id?'Menyimpan...':'✅ Tandai Dibayarkan'}
                        </button>
                      </div>
                    )
                  })()}
