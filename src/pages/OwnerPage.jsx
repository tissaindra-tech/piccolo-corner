  ...
  async function fetchLogs(date) {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('attendance')
        .select('employee_id, date, check_in, check_out, status, is_late, late_minutes, gps_dist_in, doc_url, employees!attendance_employee_id_fkey(name, role)')
        .eq('date', date)
...
function LeaveTab({ employees }) {
  ...
  async function fetchRequests() {
    const { data } = await supabase.from('leave_requests').select('*, employees!leave_requests_employee_id_fkey(name,role)').eq('status','pending').order('created_at', { ascending: false })
    setRequests(data || [])
  }
...
function ReportTab({ employees }) {
  ...
  async function fetchReport() {
    ...
    const end = month + '-31'
    const { data: rows } = await supabase.from('attendance').select('*, employees!attendance_employee_id_fkey(name,role,leave_balance)').gte('date', start).lte('date', end)
    // Group by employee
    const grouped = {}
    ;(rows || []).forEach(r => {
