// OwnerPage v4.0 - Gaji tetap prorata + Uang Kehadiran + revisi hari
...
// ===== DAILY WORKER =====
// Label & tarif default (dipakai kalau dw_rates belum termuat dari DB)
const TIPE_LABEL={tetap:'Tetap / Probation',dw_magang:'Daily Worker Magang',dw_fullday:'Daily Worker Full Day'}
const DW_RATES_DEFAULT={magang_6jam:50000,magang_8jam:70000,magang_10jam:100000,fullday_per_hari:100000}

// Hitung jam kerja dari 1 record absensi (return null jika data tidak lengkap)
function jamKerja(att){
  if(!att?.check_in||!att?.check_out) return null
  return (new Date(att.check_out)-new Date(att.check_in))/3600000
}

// Fee 1 shift untuk DW Magang -- PEMBULATAN KE BAWAH
// 6-7j=50rb | 8-9j=70rb | >=10j=100rb | <6j=0 (perlu keputusan manual)
function dwFeeForShift(att,rates){
  const r=rates||DW_RATES_DEFAULT
  if(!att||att.status!=='hadir') return {fee:0,jam:null,warn:false,noOut:false}
  if(!att.check_in||!att.check_out) return {fee:0,jam:null,warn:true,noOut:true}
  const jam=jamKerja(att)
  if(jam>=10) return {fee:parseInt(r.magang_10jam)||100000,jam,warn:false,noOut:false}
  if(jam>=8)  return {fee:parseInt(r.magang_8jam)||70000,jam,warn:false,noOut:false}
  if(jam>=6)  return {fee:parseInt(r.magang_6jam)||50000,jam,warn:false,noOut:false}
  return {fee:0,jam,warn:true,noOut:false}
}

// Rincian harian DW (untuk modal & slip cetak)
function dwBreakdown(atts,tipe,rates){
  const r=rates||DW_RATES_DEFAULT
  return (atts||[]).filter(a=>a.status==='hadir')
    .sort((a,b)=>(a.date||'').localeCompare(b.date||''))
    .map(a=>{
      if(tipe==='dw_fullday') return {date:a.date,jam:jamKerja(a),fee:parseInt(r.fullday_per_hari)||100000,warn:false,noOut:!a.check_out}
      return {date:a.date,...dwFeeForShift(a,r)}
    })
}

// Hitung gaji bulanan lengkap untuk satu karyawan
// emp: data karyawan (tipe_karyawan, gaji_pokok, tunjangan, incentive_enabled)
// atts: array data attendance bulan tsb
// potongans: array potongan_gaji bulan tsb
// bonuses: array bonus_gaji bulan tsb (bonus tidak tetap)
// rates: data tarif dw_rates (opsional, fallback ke default)
function calcGajiBulanan(emp,atts,potongans,bonuses,rates,opts){
  const tipe=emp?.tipe_karyawan||'tetap'
  const bonus_total=(bonuses||[]).reduce((s,b)=>s+(parseInt(b.nominal)||0),0)
  const potongan_total=(potongans||[]).reduce((s,p)=>s+(parseInt(p.nominal)||0),0)

  // ----- DAILY WORKER (magang / full day) -----
  // Tanpa gaji pokok, tanpa tunjangan tetap, tanpa insentif kehadiran.
  // Pendapatan murni dari absensi harian. Bonus & potongan manual tetap berlaku.
  if(tipe==='dw_magang'||tipe==='dw_fullday'){
    const r=rates||DW_RATES_DEFAULT
    let dw_total=0,dw_warn=0,dw_hari=0
    ;(atts||[]).forEach(a=>{
      if(a.status!=='hadir') return
      dw_hari++
      if(tipe==='dw_fullday'){dw_total+=parseInt(r.fullday_per_hari)||100000}
      else{const f=dwFeeForShift(a,r);dw_total+=f.fee;if(f.warn) dw_warn++}
    })
    const total_pendapatan=dw_total+bonus_total
    const total_potongan=potongan_total
    return {tipe,pokok:dw_total,tunj_total:0,bonus_total,insentif:0,potongan_total,
      total_pendapatan,total_potongan,gaji_bersih:total_pendapatan-total_potongan,
      dw_total,dw_warn,dw_hari}
  }

  // ----- KARYAWAN TETAP / PROBATION -----
  // Gaji pokok PRORATA + Uang Kehadiran per hari + Insentif.
  // Matriks status:
  //   Gaji pokok  : hadir, izin tugas, sakit, cuti  (day off & CTB TIDAK)
  //   Uang hadir  : hadir, izin tugas               (day off, sakit, cuti, CTB TIDAK)
  //   Insentif    : hadir/izin tugas sesuai program (CTB TIDAK)
  const o=opts||{}
  const ov=o.override||{}
  const pokok_penuh=parseInt(emp?.gaji_pokok)||0
  const tunj_total=totalTunjangan(emp?.tunjangan)

  // Hitung otomatis dari absensi (izin tugas = status 'hadir' + is_excused, jadi sudah termasuk)
  const auto_gp_days=(atts||[]).filter(a=>a.status==='hadir'||a.status==='sakit'||a.status==='cuti').length
  const auto_uk_days=(atts||[]).filter(a=>a.status==='hadir').length

  // Pakai revisi owner jika ada, kalau tidak pakai otomatis
  const hari_kerja_bulan=(ov.hari_kerja_bulan!=null&&ov.hari_kerja_bulan!=='')?Number(ov.hari_kerja_bulan):(o.hari_kerja_bulan||26)
  const gp_days=(ov.gp_days!=null&&ov.gp_days!=='')?Number(ov.gp_days):auto_gp_days
  const uk_days=(ov.uk_days!=null&&ov.uk_days!=='')?Number(ov.uk_days):auto_uk_days
  const rate_uk=(o.uang_kehadiran_harian!=null)?Number(o.uang_kehadiran_harian):25000

  // Gaji pokok prorata (dibatasi maksimal penuh)
  const pokok=hari_kerja_bulan>0?Math.round(pokok_penuh*Math.min(gp_days,hari_kerja_bulan)/hari_kerja_bulan):0
  const uang_kehadiran=rate_uk*uk_days

  let insentif=0
  if(emp?.incentive_enabled!==false){
    (atts||[]).forEach(a=>{insentif+=calcIncentiveRp(a)})
  }
  const total_pendapatan=pokok+tunj_total+uang_kehadiran+bonus_total+(insentif>0?insentif:0)
  const total_potongan=potongan_total+(insentif<0?Math.abs(insentif):0)
  const gaji_bersih=total_pendapatan-total_potongan
  return {tipe,pokok,gaji_pokok_penuh:pokok_penuh,gp_days,auto_gp_days,hari_kerja_bulan,
    tunj_total,uang_kehadiran,uk_days,auto_uk_days,uang_kehadiran_harian:rate_uk,
    bonus_total,insentif,potongan_total,total_pendapatan,total_potongan,gaji_bersih,
    dw_total:0,dw_warn:0,dw_hari:0}
}
...
function EmployeesTab({employees,onRefresh}){
  const [form,setForm]=useState({name:'',role:'',phone:'',pin:'',shift:'',leave_balance:12,gaji_pokok:0,tunjangan:[],tipe_karyawan:'tetap',tanggal_masuk:''})
  ...
  async function addEmp(){
    setSaving(true)
    const isDW=form.tipe_karyawan==='dw_magang'||form.tipe_karyawan==='dw_fullday'
    await supabase.from('employees').insert({
      name:form.name,role:form.role,phone:form.phone,pin:form.pin,shift:form.shift,
      leave_balance:parseInt(form.leave_balance),
      tipe_karyawan:form.tipe_karyawan||'tetap',
      tanggal_masuk:form.tanggal_masuk||null,
      gaji_pokok:isDW?0:(parseInt(form.gaji_pokok)||0),
      tunjangan:isDW?[]:(form.tunjangan||[]),
      incentive_enabled:isDW?false:true
    })
    setSaving(false);setAdding(false);setForm({name:'',role:'',phone:'',pin:'',shift:'',leave_balance:12,gaji_pokok:0,tunjangan:[],tipe_karyawan:'tetap',tanggal_masuk:''});onRefresh()
  }
  function startEdit(emp){setEditingId(emp.id);setEditForm({name:emp.name,role:emp.role,phone:emp.phone,pin:'',shift:emp.shift||'',leave_balance:emp.leave_balance,incentive_enabled:emp.incentive_enabled!==false,gaji_pokok:emp.gaji_pokok||0,tunjangan:emp.tunjangan||[],tipe_karyawan:emp.tipe_karyawan||'tetap',tanggal_masuk:emp.tanggal_masuk||''})}
  async function saveEdit(id){
    setSaving(true)
    const isDW=editForm.tipe_karyawan==='dw_magang'||editForm.tipe_karyawan==='dw_fullday'
    const u={name:editForm.name,role:editForm.role,phone:editForm.phone,shift:editForm.shift,leave_balance:parseInt(editForm.leave_balance),tipe_karyawan:editForm.tipe_karyawan||'tetap',tanggal_masuk:editForm.tanggal_masuk||null,incentive_enabled:isDW?false:editForm.incentive_enabled,gaji_pokok:isDW?0:(parseInt(editForm.gaji_pokok)||0),tunjangan:isDW?[]:(editForm.tunjangan||[])}
  ...
  async function archiveEmp(id,name){
    if(!window.confirm(`Arsipkan "${name}"?\n\n\u2022 Tidak muncul lagi di daftar karyawan aktif\n\u2022 Tidak bisa login / absen\n\u2022 SEMUA data (absensi, slip gaji, foto) TETAP TERSIMPAN\n\u2022 Bisa dipulihkan kapan saja\n\nGunakan ini saat karyawan resign / berhenti.`)) return
    await supabase.from('employees').update({is_active:false}).eq('id',id);onRefresh()
  }
  async function restoreEmp(id,name){
    if(!window.confirm(`Pulihkan "${name}" menjadi karyawan aktif kembali?`)) return
    await supabase.from('employees').update({is_active:true}).eq('id',id);onRefresh()
  }
  async function delEmp(id,name){
    if(!window.confirm(`\u26a0\ufe0f HAPUS PERMANEN "${name}"?\n\nSEMUA data absensi, slip gaji, bonus, potongan, dan catatan cuti milik "${name}" akan IKUT TERHAPUS dan TIDAK BISA dikembalikan.\n\n(Foto selfie di storage tidak ikut terhapus.)`)) return
    if(!window.confirm(`Konfirmasi sekali lagi: hapus permanen "${name}" beserta SELURUH datanya?`)) return
  ...
  return(
    <div style={{padding:'14px 14px 80px'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10}}>
        <div style={{fontSize:11,fontWeight:700,color:T.muted}}>KARYAWAN ({employees.filter(e=>!e.is_owner&&e.is_active!==false).length})</div>
      ...
      {adding&&<div style={{background:T.surface,borderRadius:16,padding:14,marginBottom:10,border:`.5px solid ${T.border}`}}>
        ...
        <div style={{marginBottom:10}}>
          <div style={{fontSize:11,color:T.muted,marginBottom:3}}>Tipe Karyawan</div>
          <select value={form.tipe_karyawan} onChange={e=>setForm(f=>({...f,tipe_karyawan:e.target.value}))} style={{...inp,cursor:'pointer'}}>
            <option value="tetap">Tetap / Probation (gaji bulanan + insentif)</option>
            <option value="dw_magang">Daily Worker Magang (gaji per jam kerja)</option>
            <option value="dw_fullday">Daily Worker Full Day (flat Rp 100.000/hari)</option>
          </select>
        </div>

        {/* Tanggal Masuk Kerja */}
        <div style={{marginBottom:10}}>
          <div style={{fontSize:11,color:T.muted,marginBottom:3}}>Tanggal Masuk Kerja (untuk prorata gaji bulan pertama)</div>
          <input type="date" value={form.tanggal_masuk} onChange={e=>setForm(f=>({...f,tanggal_masuk:e.target.value}))} style={inp}/>
          <div style={{fontSize:10,color:T.muted,marginTop:2}}>Kosongkan jika karyawan lama / sudah kerja sebulan penuh</div>
        </div>

        {/* Gaji Pokok & Tunjangan -- hanya untuk Tetap */}
        {form.tipe_karyawan==='tetap'&&(
        ...
        {/* Info tarif Daily Worker */}
        {(form.tipe_karyawan==='dw_magang'||form.tipe_karyawan==='dw_fullday')&&(
        <div style={{background:T.amberLight,border:`.5px solid ${T.amberBd}`,borderRadius:10,padding:'10px 12px',marginBottom:10,fontSize:11,color:T.amber,lineHeight:1.7}}>
          <div style={{fontWeight:700,marginBottom:4}}>📅 Gaji dihitung otomatis dari absensi</div>
          {form.tipe_karyawan==='dw_magang'
            ?<>Pembulatan ke bawah: 6–7 jam = Rp 50.000 · 8–9 jam = Rp 70.000 · ≥10 jam = Rp 100.000.<br/>Di bawah 6 jam = Rp 0 (tambah manual lewat bonus di slip gaji).</>
            :<>Flat Rp 100.000 per hari masuk (berdasarkan absensi).</>}
          <br/>Tidak ikut program insentif kehadiran.
        </div>
        )}
      ...
      <div style={{display:'flex',flexDirection:'column',gap:7}}>
        {employees.filter(e=>!e.is_owner&&e.is_active!==false).map(emp=>(
          <div key={emp.id}>
            {editingId===emp.id?(
              <div style={{background:T.purpleLight,borderRadius:16,padding:14,border:`.5px solid ${T.purpleBd}`}}>
                ...
                <div style={{marginBottom:10}}>
                  <div style={{fontSize:11,color:T.muted,marginBottom:3}}>Tipe Karyawan</div>
                  <select value={editForm.tipe_karyawan||'tetap'} onChange={e=>setEditForm(f=>({...f,tipe_karyawan:e.target.value}))} style={{...inp,cursor:'pointer'}}>
                    <option value="tetap">Tetap / Probation (gaji bulanan + insentif)</option>
                    <option value="dw_magang">Daily Worker Magang (gaji per jam kerja)</option>
                    <option value="dw_fullday">Daily Worker Full Day (flat Rp 100.000/hari)</option>
                  </select>
                ...
                {/* Tanggal Masuk Kerja */}
                <div style={{marginBottom:10}}>
                  <div style={{fontSize:11,color:T.muted,marginBottom:3}}>Tanggal Masuk Kerja</div>
                  <input type="date" value={editForm.tanggal_masuk||''} onChange={e=>setEditForm(f=>({...f,tanggal_masuk:e.target.value}))} style={inp}/>
                  <div style={{fontSize:10,color:T.muted,marginTop:2}}>Kosongkan jika karyawan lama / sudah kerja sebulan penuh</div>
                </div>
                ...
                {/* Info tarif Daily Worker */}
                {(editForm.tipe_karyawan==='dw_magang'||editForm.tipe_karyawan==='dw_fullday')&&(
                <div style={{background:T.amberLight,border:`.5px solid ${T.amberBd}`,borderRadius:10,padding:'10px 12px',marginBottom:10,fontSize:11,color:T.amber,lineHeight:1.7}}>
                  <div style={{fontWeight:700,marginBottom:4}}>📅 Gaji dihitung otomatis dari absensi</div>
                  {editForm.tipe_karyawan==='dw_magang'
                    ?<>Pembulatan ke bawah: 6–7 jam = Rp 50.000 · 8–9 jam = Rp 70.000 · ≥10 jam = Rp 100.000.<br/>Di bawah 6 jam = Rp 0 (tambah manual lewat bonus di slip gaji).</>
                    :<>Flat Rp 100.000 per hari masuk (berdasarkan absensi).</>}
                  <br/>Tidak ikut program insentif kehadiran.
                </div>
                ...
                {/* Program Insentif Toggle -- tidak berlaku untuk Daily Worker */}
                {editForm.tipe_karyawan==='tetap'&&(
              ...
              <div style={{background:T.surface,borderRadius:14,border:`.5px solid ${T.border}`,display:'flex',alignItems:'center',gap:9,padding:'11px 13px'}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:12,fontWeight:700,color:T.black,display:'flex',alignItems:'center',gap:5}}>
                    ...
                    {(emp.tipe_karyawan==='dw_magang'||emp.tipe_karyawan==='dw_fullday')&&<span style={{fontSize:9,background:T.amberLight,color:T.amber,padding:'1px 5px',borderRadius:4,fontWeight:700}}>{emp.tipe_karyawan==='dw_magang'?'DW Magang':'DW Full Day'}</span>}
                    {emp.incentive_enabled===false&&(emp.tipe_karyawan==='tetap'||!emp.tipe_karyawan)&&<span style={{fontSize:9,background:'#F3F4F6',color:T.muted,padding:'1px 5px',borderRadius:4}}>Non-program</span>}
                  </div>
                  <div style={{fontSize:10,color:T.muted}}>{emp.role} . {emp.phone} . Cuti: {emp.leave_balance}hr{emp.shift?` . ${emp.shift}`:''}</div>
                  {(emp.tipe_karyawan==='dw_magang'||emp.tipe_karyawan==='dw_fullday')
                    ?<div style={{fontSize:10,color:T.amber,fontWeight:600,marginTop:1}}>📅 Gaji dihitung dari absensi harian</div>
                ...
                <div style={{display:'flex',gap:5,flexShrink:0}}>
                  ...
                  <button onClick={()=>archiveEmp(emp.id,emp.name)} title="Arsipkan" style={{fontSize:11,padding:'4px 9px',borderRadius:7,border:`.5px solid ${T.amberBd}`,background:T.amberLight,color:T.amber,cursor:'pointer',fontFamily:'inherit',fontWeight:600}}>📦</button>
                </div>
              </div>
            )}
          </div>
        ...
        {/* ===== ARSIP KARYAWAN (karyawan yang sudah resign/diarsipkan) ===== */}
        {(()=>{
          const arsip=employees.filter(e=>!e.is_owner&&e.is_active===false)
          if(arsip.length===0) return null
          return (
            <div style={{marginTop:18}}>
              <button onClick={()=>setShowArsip(v=>!v)} style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 13px',background:T.bg,border:`.5px solid ${T.border}`,borderRadius:12,cursor:'pointer',fontFamily:'inherit'}}>
                <span style={{fontSize:12,fontWeight:700,color:T.muted}}>📦 Arsip Karyawan ({arsip.length})</span>
                <span style={{fontSize:12,color:T.muted}}>{showArsip?'▲ Tutup':'▼ Lihat'}</span>
              </button>
              {showArsip&&<div style={{marginTop:8,display:'flex',flexDirection:'column',gap:7}}>
                {arsip.map(emp=>(
                  <div key={emp.id} style={{background:T.bg,borderRadius:12,border:`.5px solid ${T.border}`,display:'flex',alignItems:'center',gap:9,padding:'10px 12px',opacity:.9}}>
                    <div style={{width:34,height:34,borderRadius:'50%',background:'#D1D5DB',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,color:'#fff',flexShrink:0,overflow:'hidden'}}>
                      {emp.photo_url?<img src={emp.photo_url} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:emp.name?.split(' ').map(w=>w[0]).join('').slice(0,2)}
                    </div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:12,fontWeight:700,color:T.black}}>{emp.name}</div>
                      <div style={{fontSize:10,color:T.muted}}>{emp.role}{emp.phone?` · ${emp.phone}`:''} · diarsipkan</div>
                    </div>
                    <div style={{display:'flex',gap:5,flexShrink:0}}>
                      <button onClick={()=>restoreEmp(emp.id,emp.name)} title="Pulihkan" style={{fontSize:11,padding:'4px 9px',borderRadius:7,border:`.5px solid ${T.greenBd}`,background:T.greenLight,color:T.green,cursor:'pointer',fontFamily:'inherit',fontWeight:700}}>↩️ Pulihkan</button>
                      <button onClick={()=>delEmp(emp.id,emp.name)} title="Hapus permanen" style={{fontSize:11,padding:'4px 9px',borderRadius:7,border:`.5px solid ${T.redBd}`,background:T.redLight,color:T.red,cursor:'pointer',fontFamily:'inherit',fontWeight:600}}>🗑</button>
                    </div>
                  </div>
                ))}
                <div style={{fontSize:10,color:T.muted,lineHeight:1.5,marginTop:2,padding:'0 2px'}}>
                  Karyawan di arsip tidak bisa login &amp; tidak dihitung sebagai staf aktif, tapi semua datanya tetap aman dan bisa dipulihkan kapan saja. Tombol 🗑 menghapus permanen (berisiko — semua data ikut hilang).
                </div>
              </div>}
            </div>
          )
...
function ReportTab({employees}){
  ...
  const [dwRates,setDwRates]=useState(DW_RATES_DEFAULT)
  const [uangKehadiran,setUangKehadiran]=useState(25000)
  const [loading,setLoading]=useState(false)

  // ----- SLIP GAJI MODAL STATE -----
  const [gajiModal,setGajiModal]=useState(null)
  const [newPotongan,setNewPotongan]=useState({tipe:'kasbon',nominal:'',catatan:''})
  const [newBonus,setNewBonus]=useState({nama:'',nominal:'',catatan:''})
  const [paidSlips,setPaidSlips]=useState({}) // key: empId_bulan, value: slip_gaji_final row
  const [payingId,setPayingId]=useState(null) // empId sedang diproses
  const [payForm,setPayForm]=useState({tgl:'',metode:'transfer'})
  ...
  // Muat tarif daily worker sekali saat tab dibuka
  useEffect(()=>{(async()=>{const {data}=await supabase.from('dw_rates').select('*').eq('id',1).single();if(data)setDwRates(data)})()},[])
  // Muat tarif uang kehadiran harian dari pengaturan
  useEffect(()=>{(async()=>{const {data}=await supabase.from('work_settings').select('uang_kehadiran_harian').eq('id',1).single();if(data&&data.uang_kehadiran_harian!=null)setUangKehadiran(Number(data.uang_kehadiran_harian))})()},[])

  // Pembagi hari kerja bulan itu = jumlah hari kalender - 4 hari day off (otomatis)
  function hariKerjaBulan(bulanStr){
    const [y,m]=(bulanStr||'').split('-').map(Number)
    if(!y||!m) return 26
    const daysInMonth=new Date(y,m,0).getDate()
    return daysInMonth-4
  }
  // Bangun opsi perhitungan gaji tetap (pembagi + tarif UK + revisi manual owner)
  function gajiOpts(bulanStr,override){
    return {hari_kerja_bulan:hariKerjaBulan(bulanStr),uang_kehadiran_harian:uangKehadiran,override:override||null}
  }

  async function fetchData(){
    setLoading(true)
    const {from,to}=getDateRange()
    let q=supabase.from('attendance').select('*,employees!employee_id(name,role,leave_balance,incentive_enabled,gaji_pokok,tunjangan,tipe_karyawan)').gte('date',from).lte('date',to).order('date',{ascending:false})
    ...
    setPotongans(pdata||[])
    // Fetch bonus tidak tetap
    let bq=supabase.from('bonus_gaji').select('*').gte('bulan',fromMonth).lte('bulan',toMonth)
    if(filterEmpId!=='all') bq=bq.eq('employee_id',filterEmpId)
    const {data:bdata}=await bq
    setBonuses(bdata||[])
    // Fetch slip gaji yang sudah dibayarkan
    const {data:pdata2}=await supabase.from('slip_gaji_final').select('*').gte('bulan',fromMonth).lte('bulan',toMonth)
    const pmap={}
    ;(pdata2||[]).forEach(p=>{pmap[p.employee_id+'_'+p.bulan]=p})
    setPaidSlips(pmap)
    setLoading(false)
  }
  ...
  function groupByEmployee(arr){
    ...
    arr.forEach(r=>{
      ...
      if(!grouped[id]){
        grouped[id]={
          ...
          tipe_karyawan:r.employees?.tipe_karyawan||'tetap',
    ...
    Object.keys(grouped).forEach(id=>{
      ...
      grouped[id]._bonuses=bonuses.filter(b=>b.employee_id===id)
    })
  ...
  function exportExcel(){
    ...
    if(view==='gaji'||filterEmpId!=='all'){
      const bulanX=getDateRange().from.slice(0,7)
      const ws4=[
        [`SLIP GAJI BULANAN - ${label}`],
        [],
        ['Nama','Jabatan','Tipe','Gaji Pokok / Upah','Tunjangan Tetap','Uang Kehadiran','Bonus Tidak Tetap','Insentif','Total Pendapatan','Potongan','Gaji Bersih']
      ]
      empData.forEach(d=>{
        const empPotongs=potongans.filter(p=>p.employee_id===d.id)
        const empBonuses=bonuses.filter(b=>b.employee_id===d.id)
        const calc=calcGajiBulanan({gaji_pokok:d.gaji_pokok,tunjangan:d.tunjangan,incentive_enabled:d.incentive_enabled,tipe_karyawan:d.tipe_karyawan},d._atts,empPotongs,empBonuses,dwRates,gajiOpts(bulanX))
        const isDW=d.tipe_karyawan==='dw_magang'||d.tipe_karyawan==='dw_fullday'
        const bonusNames=empBonuses.length>0?empBonuses.map(b=>b.nama+'(Rp '+parseInt(b.nominal).toLocaleString('id-ID')+')').join(', '):'—'
        ws4.push([d.name,d.role,TIPE_LABEL[d.tipe_karyawan||'tetap'],'Rp '+calc.pokok.toLocaleString('id-ID'),'Rp '+calc.tunj_total.toLocaleString('id-ID'),isDW?'—':'Rp '+(calc.uang_kehadiran||0).toLocaleString('id-ID'),'Rp '+calc.bonus_total.toLocaleString('id-ID')+' | '+bonusNames,isDW?'—':(calc.insentif>=0?'Rp ':'-Rp ')+Math.abs(calc.insentif).toLocaleString('id-ID'),'Rp '+calc.total_pendapatan.toLocaleString('id-ID'),'Rp '+calc.total_potongan.toLocaleString('id-ID'),'Rp '+calc.gaji_bersih.toLocaleString('id-ID')])
      })
      const sht4=XLSX.utils.aoa_to_sheet(ws4)
      sht4['!cols']=[{wch:22},{wch:14},{wch:20},{wch:16},{wch:14},{wch:16},{wch:24},{wch:14},{wch:16},{wch:14},{wch:16}]
  ...
  // ===== BONUS MANAGEMENT =====
  async function addBonus(empId,bulan){
    if(!newBonus.nama||!newBonus.nominal) return
    await supabase.from('bonus_gaji').insert({employee_id:empId,bulan,nama:newBonus.nama,nominal:parseInt(newBonus.nominal),catatan:newBonus.catatan})
    setNewBonus({nama:'',nominal:'',catatan:''})
    fetchData()
    if(gajiModal&&gajiModal.emp.id===empId){
      const {data}=await supabase.from('bonus_gaji').select('*').eq('employee_id',empId).eq('bulan',bulan)
      setGajiModal({...gajiModal,bonuses:data||[]})
    }
  }
  async function delBonus(id,empId,bulan){
    if(!window.confirm('Hapus bonus ini?')) return
    await supabase.from('bonus_gaji').delete().eq('id',id)
    fetchData()
    if(gajiModal&&gajiModal.emp.id===empId){
      const {data}=await supabase.from('bonus_gaji').select('*').eq('employee_id',empId).eq('bulan',bulan)
      setGajiModal({...gajiModal,bonuses:data||[]})
  ...
  async function batalkanBayar(){
    if(!gajiModal) return
    const {emp,bulan,label}=gajiModal
    if(!window.confirm(`Batalkan pembayaran gaji ${emp.name} bulan ${label}?\n\nSlip gaji akan disembunyikan dari karyawan dan bisa diedit ulang.\n\nLakukan ini hanya jika ada kesalahan input.`)) return
    setPayingId(emp.id)
    await supabase.from('slip_gaji_final').delete().eq('employee_id',emp.id).eq('bulan',bulan)
    setPayingId(null)
    fetchData()
    const pkey=emp.id+'_'+bulan
    setPaidSlips(prev=>{const n={...prev};delete n[pkey];return n})
    alert(`Pembayaran dibatalkan. Slip gaji ${emp.name} bulan ${label} sudah disembunyikan dari karyawan.\n\nSilakan perbaiki data lalu klik "Tandai Dibayarkan" kembali.`)
  }
  async function tandaiBayar(){
    if(!gajiModal) return
    // Validasi form dulu
    if(!payForm.tgl){alert('Harap isi tanggal pembayaran terlebih dahulu');return}
    const {emp,atts,potongans:pots,bonuses:bons,bulan,label}=gajiModal
    const calc=calcGajiBulanan(emp,atts,pots,bons,dwRates,gajiOpts(bulan,gajiModal.override))
    const tunj=Array.isArray(emp.tunjangan)?emp.tunjangan:[]
    const tglFmt=new Date(payForm.tgl).toLocaleDateString('id-ID',{day:'numeric',month:'long',year:'numeric'})
    const metodeFmt=payForm.metode==='transfer'?'Transfer Bank':'Tunai (Cash)'
    if(!window.confirm(`Konfirmasi pembayaran gaji:\n\nKaryawan : ${emp.name}\nBulan    : ${label}\nGaji Bersih : Rp ${calc.gaji_bersih.toLocaleString('id-ID')}\nDibayar tgl : ${tglFmt}\nMetode   : ${metodeFmt}\n\nSetelah dikonfirmasi, slip gaji akan terlihat oleh karyawan.`)) return
    setPayingId(emp.id)
    const dibayarkan_at=new Date(payForm.tgl).toISOString()
    // Buat snapshot lengkap — termasuk tanggal bayar + metode
    const snapshot={
      nama:emp.name, role:emp.role, bulan, label,
      tipe_karyawan:emp.tipe_karyawan||'tetap',
      gaji_pokok:calc.pokok,
      gaji_pokok_penuh:calc.gaji_pokok_penuh||calc.pokok,
      gp_days:calc.gp_days, hari_kerja_bulan:calc.hari_kerja_bulan,
      uang_kehadiran:calc.uang_kehadiran||0, uk_days:calc.uk_days||0,
      uang_kehadiran_harian:calc.uang_kehadiran_harian||0,
      tunjangan:tunj,
      tunj_total:calc.tunj_total,
      bonuses:bons,
      bonus_total:calc.bonus_total,
      insentif:calc.insentif,
      potongan_manual:pots,
      potongan_total:calc.potongan_total,
      total_pendapatan:calc.total_pendapatan,
      total_potongan:calc.total_potongan,
      gaji_bersih:calc.gaji_bersih,
      hari_hadir:(atts||[]).filter(a=>a.status==='hadir').length,
      dw_total:calc.dw_total||0,
      dw_warn:calc.dw_warn||0,
      tgl_bayar:payForm.tgl,
      metode_bayar:payForm.metode,
      dibayarkan_at
    }
    await supabase.from('slip_gaji_final').upsert({
      employee_id:emp.id, bulan, snapshot, gaji_bersih:calc.gaji_bersih,
      dibayarkan_at
    },{onConflict:'employee_id,bulan'})
    setPayingId(null)
    setShowPayForm(false)
    setPayForm({tgl:'',metode:'transfer'})
    fetchData()
    const pkey=emp.id+'_'+bulan
    setPaidSlips(prev=>({...prev,[pkey]:{employee_id:emp.id,bulan,snapshot,gaji_bersih:calc.gaji_bersih,dibayarkan_at}}))
    if(window.confirm('Berhasil! Slip gaji sudah dipublish ke karyawan.\n\nCetak slip gaji sekarang?')) printSlipGaji()
  }
  ...
  async function openGajiModal(emp){
    ...
    const {data:bons}=await supabase.from('bonus_gaji').select('*').eq('employee_id',emp.id).eq('bulan',bulan)
    setGajiModal({emp,atts:atts||[],potongans:pots||[],bonuses:bons||[],bulan,label:getDateRange().label,override:{}})
  }
  ...
  function printSlipGaji(){
    ...
    const calc=calcGajiBulanan(emp,atts,pots,bons,dwRates,gajiOpts(bulan,gajiModal.override))
    const tunj=Array.isArray(emp.tunjangan)?emp.tunjangan:[]
    const isDW=emp.tipe_karyawan==='dw_magang'||emp.tipe_karyawan==='dw_fullday'
    const dwRows=isDW?dwBreakdown(atts,emp.tipe_karyawan,dwRates):[]
    const totalKerja=atts.filter(a=>a.status==='hadir').length
    // Ambil tanggal bayar + metode dari paidSlips kalau sudah dibayar
    const pkey=emp.id+'_'+bulan
    const paidSnap=paidSlips[pkey]?.snapshot
    const tglBayarCetak=paidSnap?.tgl_bayar?new Date(paidSnap.tgl_bayar).toLocaleDateString('id-ID',{day:'numeric',month:'long',year:'numeric'}):''
    const metodeCetak=paidSnap?.metode_bayar==='cash'?'Tunai (Cash)':'Transfer Bank'
...
<div class="section">
  ...
  ${isDW
    ?`<div class="row" style="color:#666;font-size:11px"><span>${TIPE_LABEL[emp.tipe_karyawan]} — ${dwRows.length} hari kerja</span><span></span></div>`
      +dwRows.map(r=>`<div class="row"${r.warn?' style="color:#DC2626"':''}><span>${new Date(r.date).toLocaleDateString('id-ID',{day:'2-digit',month:'short'})}${r.jam!=null?` &middot; ${r.jam.toFixed(1)} jam`:r.noOut?' &middot; belum clock-out':''}${r.warn?' &#9888;':''}</span><span>Rp ${(r.fee||0).toLocaleString('id-ID')}</span></div>`).join('')
      +`<div class="row" style="font-weight:600;border-top:1px dashed #ddd;padding-top:5px"><span>Subtotal Upah Harian</span><span>Rp ${calc.dw_total.toLocaleString('id-ID')}</span></div>`
    :`<div class="row"><span>Gaji Pokok (${calc.gp_days}/${calc.hari_kerja_bulan} hari)</span><span>Rp ${calc.pokok.toLocaleString('id-ID')}</span></div>`
      +tunj.map(t=>`<div class="row"><span>Tunjangan ${t.nama||'-'}</span><span>Rp ${(parseInt(t.nominal)||0).toLocaleString('id-ID')}</span></div>`).join('')
      +`<div class="row"><span>Uang Kehadiran (${calc.uk_days} &times; Rp ${(calc.uang_kehadiran_harian||0).toLocaleString('id-ID')})</span><span>Rp ${(calc.uang_kehadiran||0).toLocaleString('id-ID')}</span></div>`
  }
  ${(bons||[]).map(b=>`<div class="row" style="color:#16A34A"><span>${b.nama||'Bonus'}${b.catatan?` (${b.catatan})`:''}</span><span>+ Rp ${(parseInt(b.nominal)||0).toLocaleString('id-ID')}</span></div>`).join('')}
...
<div class="bersih">
  ...
  <div style="text-align:right;font-size:11px;color:#16A34A"><div>${tglBayarCetak||'—'}</div><div style="font-weight:700">${metodeCetak}</div></div>
</div>
<div class="foot">
      ...
      {!loading&&view==='gaji'&&empData.length>0&&(
        <div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:6}}>
            {empData.map(d=>{
              const empPotongs=potongans.filter(p=>p.employee_id===d.id)
              const empBonuses=bonuses.filter(b=>b.employee_id===d.id).concat(d._bonuses||[]).filter((b,i,arr)=>arr.findIndex(x=>x.id===b.id)===i)
              const bulan=getDateRange().from.slice(0,7)
              const calc=calcGajiBulanan({gaji_pokok:d.gaji_pokok,tunjangan:d.tunjangan,incentive_enabled:d.incentive_enabled,tipe_karyawan:d.tipe_karyawan},d._atts,empPotongs,empBonuses,dwRates,gajiOpts(bulan))
              const isDW=d.tipe_karyawan==='dw_magang'||d.tipe_karyawan==='dw_fullday'
              const hasGaji=isDW||d.gaji_pokok>0||totalTunjangan(d.tunjangan)>0
              const pkey=d.id+'_'+bulan
              const isPaid=!!paidSlips[pkey]
              const paidAt=isPaid?new Date(paidSlips[pkey].dibayarkan_at).toLocaleDateString('id-ID',{day:'numeric',month:'short',year:'numeric'}):null
              return(
                <div key={d.id} onClick={()=>openGajiModal(d)} style={{background:T.surface,borderRadius:12,border:`.5px solid ${isPaid?T.greenBd:T.border}`,padding:'11px 13px',cursor:'pointer'}}>
                  <div style={{display:'flex',alignItems:'center',gap:9,marginBottom:hasGaji?9:0}}>
                    <div style={{width:32,height:32,borderRadius:'50%',background:'#D1D5DB',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,color:'#fff',flexShrink:0}}>
                      {d.name?.split(' ').map(w=>w[0]).join('').slice(0,2)}
                    </div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:12,fontWeight:700,color:T.black,display:'flex',alignItems:'center',gap:5}}>{d.name}
                        {isDW&&<span style={{fontSize:9,background:T.amberLight,color:T.amber,padding:'1px 5px',borderRadius:4,fontWeight:700}}>{d.tipe_karyawan==='dw_magang'?'DW Magang':'DW Full'}</span>}
                      </div>
                      <div style={{fontSize:10,color:T.muted}}>{d.role}{isDW?` · ${d.hadir} hari kerja`:''}</div>
                    </div>
                    {isPaid
                      ?<span style={{fontSize:9,padding:'3px 8px',background:T.greenLight,color:T.green,borderRadius:6,fontWeight:700,flexShrink:0,whiteSpace:'nowrap'}}>✅ {paidAt}</span>
                      :hasGaji
                        ?<span style={{fontSize:9,padding:'3px 8px',background:'#FEF3C7',color:'#92400E',borderRadius:6,fontWeight:700,flexShrink:0}}>Belum bayar</span>
                        :<span style={{fontSize:9,padding:'3px 8px',background:T.amberLight,color:T.amber,borderRadius:6,fontWeight:600}}>Belum set gaji</span>
                    }
                  </div>
                  {hasGaji&&(
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr 1fr',gap:6,padding:'8px 0 0',borderTop:`.5px dashed ${T.border}`}}>
                      <div><div style={{fontSize:8,color:T.muted,letterSpacing:'.04em'}}>{isDW?'UPAH HARIAN':'POKOK+UK'}</div><div style={{fontSize:11,fontWeight:700,color:T.black}}>Rp {(calc.pokok+calc.tunj_total+(calc.uang_kehadiran||0)).toLocaleString('id-ID')}</div></div>
                      <div><div style={{fontSize:8,color:T.blue,letterSpacing:'.04em',fontWeight:700}}>BONUS</div><div style={{fontSize:11,fontWeight:700,color:calc.bonus_total>0?T.blue:T.muted}}>{calc.bonus_total>0?'Rp '+calc.bonus_total.toLocaleString('id-ID'):'—'}</div></div>
                      <div><div style={{fontSize:8,color:T.muted,letterSpacing:'.04em'}}>INSENTIF</div><div style={{fontSize:11,fontWeight:700,color:isDW?T.muted:(calc.insentif>=0?T.green:T.red)}}>{isDW?'—':(calc.insentif>=0?'+':'-')+'Rp '+Math.abs(calc.insentif).toLocaleString('id-ID')}</div></div>
                  ...
                  {!isDW&&hasGaji&&<div style={{marginTop:6,fontSize:9,color:T.muted}}>📊 Gaji pokok {calc.gp_days}/{calc.hari_kerja_bulan} hari = {rp(calc.pokok)} · Uang hadir {calc.uk_days} hari = {rp(calc.uang_kehadiran)}</div>}
                  {isDW&&calc.dw_warn>0&&<div style={{marginTop:6,fontSize:10,color:T.red,fontWeight:600}}>⚠ {calc.dw_warn} hari di bawah 6 jam → fee Rp 0, perlu keputusan manual</div>}
                </div>
              )
            })}
          </div>
      ...
      {gajiModal&&(
        <div onClick={()=>setGajiModal(null)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.6)',zIndex:50,display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
          <div onClick={e=>e.stopPropagation()} style={{background:T.surface,borderRadius:'18px 18px 0 0',width:'100%',maxWidth:520,maxHeight:'90vh',overflowY:'auto',padding:'16px 14px 80px'}}>
            ...
            {(() => {
              const isDW=gajiModal.emp.tipe_karyawan==='dw_magang'||gajiModal.emp.tipe_karyawan==='dw_fullday'
              const calc=calcGajiBulanan(gajiModal.emp,gajiModal.atts,gajiModal.potongans,gajiModal.bonuses||[],dwRates,gajiOpts(gajiModal.bulan,gajiModal.override))
              const tunj=Array.isArray(gajiModal.emp.tunjangan)?gajiModal.emp.tunjangan:[]
              const bons=gajiModal.bonuses||[]
              const dwRows=isDW?dwBreakdown(gajiModal.atts,gajiModal.emp.tipe_karyawan,dwRates):[]
              const setOv=(k,v)=>setGajiModal(m=>({...m,override:{...(m.override||{}),[k]:v}}))
              const ovVal=(k,auto)=>{const o=gajiModal.override||{};return (o[k]!=null&&o[k]!=='')?o[k]:auto}
              const inpMini={width:52,padding:'3px 6px',border:`.5px solid ${T.border}`,borderRadius:6,fontSize:11,background:'#fff',color:T.black,fontFamily:'inherit',textAlign:'center',fontWeight:700}
              return(
                <>
                  {/* PENDAPATAN */}
                  <div style={{background:T.greenLight,border:`.5px solid ${T.greenBd}`,borderRadius:10,padding:'10px 12px',marginBottom:10}}>
                    ...
                    {isDW?(
                      <>
                        <div style={{fontSize:10,color:T.muted,marginBottom:4}}>{TIPE_LABEL[gajiModal.emp.tipe_karyawan]} · {dwRows.length} hari kerja</div>
                        {dwRows.length===0&&<div style={{fontSize:11,color:T.muted,fontStyle:'italic',padding:'2px 0'}}>Belum ada hari kerja pada periode ini</div>}
                        {dwRows.map((row,i)=>(
                          <div key={i} style={{display:'flex',justifyContent:'space-between',fontSize:11,padding:'2px 0',color:row.warn?T.red:T.black}}>
                            <span>{new Date(row.date).toLocaleDateString('id-ID',{day:'2-digit',month:'short'})}{row.jam!=null?` · ${row.jam.toFixed(1)} jam`:row.noOut?' · belum clock-out':''}{row.warn?' ⚠':''}</span>
                            <span>Rp {(row.fee||0).toLocaleString('id-ID')}</span>
                          </div>
                        ))}
                        <div style={{display:'flex',justifyContent:'space-between',fontSize:11,padding:'6px 0 2px',borderTop:`.5px dashed ${T.greenBd}`,marginTop:4,fontWeight:600}}><span>Subtotal Upah Harian</span><span>Rp {calc.dw_total.toLocaleString('id-ID')}</span></div>
                      </>
                    ):(
                      <>
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',fontSize:11,padding:'3px 0'}}>
                          <span style={{display:'flex',alignItems:'center',gap:4,flexWrap:'wrap'}}>Gaji Pokok
                            <input type="number" inputMode="numeric" value={ovVal('gp_days',calc.auto_gp_days)} onChange={e=>setOv('gp_days',e.target.value)} style={inpMini}/>
                            <span style={{color:T.muted}}>÷</span>
                            <input type="number" inputMode="numeric" value={ovVal('hari_kerja_bulan',calc.hari_kerja_bulan)} onChange={e=>setOv('hari_kerja_bulan',e.target.value)} style={inpMini}/>
                            <span style={{color:T.muted}}>hari</span>
                          </span>
                          <span style={{fontWeight:700}}>Rp {calc.pokok.toLocaleString('id-ID')}</span>
                        </div>
                        <div style={{fontSize:9,color:T.muted,marginBottom:2}}>dari {rp(calc.gaji_pokok_penuh)}/bulan (otomatis: {calc.auto_gp_days} hari kerja dibayar)</div>
                        ...
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',fontSize:11,padding:'3px 0'}}>
                          <span style={{display:'flex',alignItems:'center',gap:4,flexWrap:'wrap'}}>Uang Kehadiran
                            <input type="number" inputMode="numeric" value={ovVal('uk_days',calc.auto_uk_days)} onChange={e=>setOv('uk_days',e.target.value)} style={inpMini}/>
                            <span style={{color:T.muted}}>× {rp(calc.uang_kehadiran_harian)}</span>
                          </span>
                          <span style={{fontWeight:700}}>Rp {calc.uang_kehadiran.toLocaleString('id-ID')}</span>
                        </div>
                      </>
                    )}
                    {bons.map(b=>(<div key={b.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',fontSize:11,padding:'2px 0',color:T.blue}}><span style={{flex:1}}>🎁 {b.nama}{b.catatan?` (${b.catatan})`:''}</span><span>+ Rp {(parseInt(b.nominal)||0).toLocaleString('id-ID')}</span><button onClick={()=>delBonus(b.id,gajiModal.emp.id,gajiModal.bulan)} style={{marginLeft:6,padding:'2px 6px',fontSize:9,background:'transparent',border:`.5px solid #BFDBFE`,color:T.blue,borderRadius:4,cursor:'pointer'}}>×</button></div>))}
                    {calc.insentif>0&&<div style={{display:'flex',justifyContent:'space-between',fontSize:11,padding:'2px 0',color:T.green}}><span>Insentif Kehadiran</span><span>+ Rp {calc.insentif.toLocaleString('id-ID')}</span></div>}
                    <div style={{display:'flex',justifyContent:'space-between',fontSize:12,padding:'6px 0 2px',borderTop:`.5px solid ${T.greenBd}`,marginTop:4,fontWeight:700}}><span>Total Pendapatan</span><span>Rp {calc.total_pendapatan.toLocaleString('id-ID')}</span></div>
                  </div>

                  {/* Catatan revisi hari (tetap) */}
                  {!isDW&&<div style={{background:T.blueLight,border:`.5px solid ${T.blueBd}`,borderRadius:10,padding:'8px 12px',marginBottom:10,fontSize:10,color:T.blue,lineHeight:1.5}}>✏️ Angka <strong>hari</strong> di atas dihitung otomatis dari absensi, tapi bisa Tissa <strong>revisi manual</strong> (ketik ulang di kotak putih) kalau ada kesalahan teknis. Kosongkan kotak untuk kembali ke hitungan otomatis.</div>}

                  {/* Peringatan hari kerja < 6 jam (DW Magang) */}
                  {isDW&&calc.dw_warn>0&&<div style={{background:T.amberLight,border:`.5px solid ${T.amberBd}`,borderRadius:10,padding:'8px 12px',marginBottom:10,fontSize:11,color:T.amber,lineHeight:1.5}}>⚠ Ada {calc.dw_warn} hari kerja di bawah 6 jam (fee Rp 0). Jika karyawan tetap berhak dibayar, tambahkan lewat "Tambah Bonus" di bawah.</div>}

                  {/* TAMBAH BONUS MANUAL */}
                  <div style={{background:'#EFF6FF',border:'.5px solid #BFDBFE',borderRadius:10,padding:'10px 12px',marginBottom:10}}>
                    <div style={{fontSize:10,fontWeight:700,color:T.blue,marginBottom:6,letterSpacing:'.04em'}}>🎁 + TAMBAH BONUS / TUNJANGAN TIDAK TETAP</div>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6,marginBottom:6}}>
                      <input value={newBonus.nama} onChange={e=>setNewBonus(b=>({...b,nama:e.target.value}))} placeholder="Nama (cth: THR, Lembur)" style={inp}/>
                      <input type="number" inputMode="numeric" value={newBonus.nominal} onChange={e=>setNewBonus(b=>({...b,nominal:e.target.value}))} placeholder="Nominal Rp" style={inp}/>
                    </div>
                    <input value={newBonus.catatan} onChange={e=>setNewBonus(b=>({...b,catatan:e.target.value}))} placeholder="Catatan (opsional)" style={{...inp,width:'100%',marginBottom:6}}/>
                    <button onClick={()=>addBonus(gajiModal.emp.id,gajiModal.bulan)} disabled={!newBonus.nama||!newBonus.nominal} style={{width:'100%',padding:8,background:'#2563EB',color:'#fff',border:'none',borderRadius:7,fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'inherit',opacity:(newBonus.nama&&newBonus.nominal)?1:.5}}>+ Tambah Bonus</button>
                  </div>
                  ...
                  {/* Footer buttons */}
                  {(()=>{
                    const pkey=gajiModal.emp.id+'_'+gajiModal.bulan
                    const isPaidModal=!!paidSlips[pkey]
                    const snapPaid=isPaidModal?paidSlips[pkey].snapshot:null
                    const tglBayar=snapPaid?.tgl_bayar?new Date(snapPaid.tgl_bayar).toLocaleDateString('id-ID',{day:'numeric',month:'long',year:'numeric'}):isPaidModal?new Date(paidSlips[pkey].dibayarkan_at).toLocaleDateString('id-ID',{day:'numeric',month:'long',year:'numeric'}):null
                    const metodeBayar=snapPaid?.metode_bayar==='cash'?'Tunai (Cash)':'Transfer Bank'
                    return isPaidModal?(
                      <>
                        <div style={{background:T.greenLight,border:`.5px solid ${T.greenBd}`,borderRadius:10,padding:'10px 14px',marginBottom:10,display:'flex',alignItems:'center',gap:8}}>
                          <span style={{fontSize:18}}>✅</span>
                          <div style={{flex:1}}>
                            <div style={{fontSize:12,fontWeight:700,color:T.green}}>Sudah Dibayarkan</div>
                            <div style={{fontSize:10,color:T.green}}>{tglBayar} via {metodeBayar}</div>
                            <div style={{fontSize:10,color:T.green,marginTop:1}}>Slip gaji sudah terlihat oleh karyawan</div>
                          </div>
                        </div>
                        <div style={{display:'flex',gap:7}}>
                        ...
                        <button onClick={batalkanBayar} disabled={payingId===gajiModal.emp.id} style={{width:'100%',marginTop:8,padding:9,background:'transparent',color:T.red,border:`.5px solid ${T.redBd}`,borderRadius:9,fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'inherit',opacity:payingId===gajiModal.emp.id?.5:1}}>
                          {payingId===gajiModal.emp.id?'Membatalkan...':'↩ Batalkan Pembayaran (jika ada kesalahan input)'}
                        </button>
                      </>
                    ):(
                      <>
                        {showPayForm?(
                          <div style={{background:'#F0FDF4',border:`.5px solid ${T.greenBd}`,borderRadius:12,padding:'12px 14px',marginBottom:10}}>
                            <div style={{fontSize:11,fontWeight:700,color:T.green,marginBottom:10,letterSpacing:'.04em'}}>✅ KONFIRMASI PEMBAYARAN</div>
                            <div style={{marginBottom:8}}>
                              <div style={{fontSize:10,color:T.muted,marginBottom:4}}>TANGGAL DIBAYARKAN <span style={{color:T.red}}>*</span></div>
                              <input type="date" value={payForm.tgl} onChange={e=>setPayForm(f=>({...f,tgl:e.target.value}))}
                                style={{...inp,width:'100%',fontWeight:600,borderColor:payForm.tgl?T.greenBd:T.border}}/>
                              {!payForm.tgl&&<div style={{fontSize:10,color:T.red,marginTop:3}}>Wajib diisi</div>}
                            </div>
                            <div style={{marginBottom:12}}>
                              <div style={{fontSize:10,color:T.muted,marginBottom:4}}>METODE PEMBAYARAN</div>
                              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>
                                {[['transfer','🏦 Transfer Bank'],['cash','💵 Tunai (Cash)']].map(([val,lbl])=>(
                                  <button key={val} onClick={()=>setPayForm(f=>({...f,metode:val}))}
                                    style={{padding:'9px 8px',fontSize:11,fontWeight:700,borderRadius:9,cursor:'pointer',fontFamily:'inherit',
                                      background:payForm.metode===val?T.green:'#fff',
                                      color:payForm.metode===val?'#fff':T.muted,
                                      border:`.5px solid ${payForm.metode===val?T.green:T.border}`}}>
                                    {lbl}
                                  </button>
                                ))}
                              </div>
                            ...
                            <div style={{display:'flex',gap:7}}>
                              <button onClick={()=>{setShowPayForm(false);setPayForm({tgl:'',metode:'transfer'})}}
                                style={{flex:1,padding:10,background:'#fff',color:T.muted,border:`.5px solid ${T.border}`,borderRadius:9,fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>Batal</button>
                              <button onClick={tandaiBayar} disabled={!payForm.tgl||payingId===gajiModal.emp.id}
                                style={{flex:2,padding:10,background:T.green,color:'#fff',border:'none',borderRadius:9,fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'inherit',opacity:(!payForm.tgl||payingId===gajiModal.emp.id)?.5:1}}>
                                {payingId===gajiModal.emp.id?'Menyimpan...':'Konfirmasi & Publish'}
                              </button>
                            </div>
                          </div>
                        ):(
                          <div style={{display:'flex',gap:7}}>
                            <button onClick={()=>setGajiModal(null)} style={{flex:1,padding:11,background:T.bg,color:T.black,border:`.5px solid ${T.border}`,borderRadius:9,fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>Tutup</button>
                            <button onClick={printSlipGaji} style={{flex:1,padding:11,background:T.bg,color:T.black,border:`.5px solid ${T.border}`,borderRadius:9,fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>🖨 Cetak</button>
                            <button onClick={()=>setShowPayForm(true)}
                              style={{flex:2,padding:11,background:T.green,color:'#fff',border:'none',borderRadius:9,fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>
                              ✅ Tandai Dibayarkan
                            </button>
                          </div>
                        )}
