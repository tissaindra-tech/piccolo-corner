        ...
        {tab === 'home' && <>
          ...
          {gpsState==='blocked' && (
            <Card style={{ textAlign: 'center', padding: '24px 16px' }}>
              ...
              <div style={{ fontSize: 12, color: T.muted, lineHeight: 1.6 }}>Datang ke Piccolo Corner dulu ya, baru bisa check in 👋</div>
              {gpsInfo?.dist && (
                <div style={{ marginTop: 12, display: 'inline-block', background: T.redLight, borderRadius: 10, padding: '6px 16px' }}>
                  <div style={{ fontSize: 20, fontWeight: 800, color: T.red }}>{formatDistance(gpsInfo.dist)}</div>
                ...
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    ...
                    <button disabled={!canCheckOut} onClick={() => startCamera('checkout')}
                      style={{ padding: '12px', background: canCheckOut ? T.surface : T.grayLight, color: canCheckOut ? T.black : T.muted, border: `.5px solid ${canCheckOut ? T.border : T.grayBd}`, borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: canCheckOut ? 'pointer' : 'not-allowed', fontFamily: 'inherit' }}>
                      ← Check Out
                    </button>
                  </div>
