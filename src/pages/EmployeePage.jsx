...
export default function EmployeePage() {
  ...
  const [backdateError, setBackdateError] = useState(null)
  ...
  async function submitBackdate() {
    if (!backdateDate) { setBackdateError('Pilih tanggal dulu'); return }
    setBackdateSubmitting(true)
    setBackdateError(null)

    // Check if record already exists
    const { data: existing } = await supabase.from('attendance')
      .select('id, status').eq('employee_id', user.id).eq('date', backdateDate).single()

    if (existing && existing.status !== 'ctb') {
      setBackdateError(`Sudah ada catatan untuk tanggal ini (${existing.status.toUpperCase()}). Tidak bisa diubah.`)
    ...
    if (backdateDoc) {
      ...
      const path = `docs/${user.id}/sakit_${backdateDate}_${Date.now()}.${ext}`
      const { data } = await supabase.storage.from('documents').upload(path, backdateDoc, { upsert: true })
      if (data) {
    ...
    if (existing) {
      // Update existing CTB → sakit
      await supabase.from('attendance').update({
        status: 'sakit', note: backdateNote, doc_url: docUrl, doc_status: docUrl ? 'pending' : 'missing'
      }).eq('id', existing.id)
    } else {
      // Insert new
      await supabase.from('attendance').insert({
        employee_id: user.id, date: backdateDate, status: 'sakit',
        note: backdateNote, doc_url: docUrl, doc_status: docUrl ? 'pending' : 'missing'
      })
    }

    setBackdateSubmitting(false)
    setBackdateModal(false)
    setBackdateDate(''); setBackdateNote(''); setBackdateDoc(null)
    showToast('ok', `Sakit ${backdateDate} berhasil dicatat${docUrl ? ' + dokumen diupload' : ''}`)
    fetchAll()
  }
  ...
  return (
    <div style={s.app}>
      ...
      <div style={s.scroll}>
        ...
        {tab === 'home' && <>
          ...
          <div style={{ marginTop: 10 }}>
            <button onClick={() => { setBackdateModal(true); setBackdateError(null) }}
              style={{ width: '100%', padding: '11px', background: 'transparent', border: `.5px dashed ${T.border}`, borderRadius: 12, fontSize: 12, fontWeight: 700, color: T.muted, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7 }}>
      ...
      {/* BACKDATE MODAL */}
      {backdateModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'flex-end', zIndex: 1001 }}>
          <div style={{ background: T.white, borderRadius: '24px 24px 0 0', padding: '20px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
            {/* Header */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              <div style={{ width: 40, height: 40, borderRadius: 12, background: T.purpleLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>🏥</div>
              <div>
                <div style={{ fontSize: 16, fontWeight: 800, color: T.black }}>Lapor Sakit Kemarin</div>
                <div style={{ fontSize: 11, color: T.muted }}>Backdate maks. {settings.doc_upload_deadline_days || 3} hari ke belakang</div>
              </div>
            </div>
            ...
            <div style={{ background: T.amberLight, borderRadius: 12, padding: '10px 14px', fontSize: 11, color: '#8B5800', marginBottom: 14, lineHeight: 1.6 }}>
              ⚠ Jika tanggal tersebut sudah tercatat sebagai <strong>CTB otomatis</strong>, statusnya akan diubah menjadi <strong>Sakit</strong> setelah kamu submit.
            </div>

            {/* Date picker */}
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: T.muted, marginBottom: 6 }}>Tanggal Sakit</div>
              <input
                type="date"
                value={backdateDate}
                ...
                onChange={e => { setBackdateDate(e.target.value); setBackdateError(null) }}
                style={{ width: '100%', padding: '11px 12px', border: `.5px solid ${T.border}`, borderRadius: 12, fontSize: 14, fontWeight: 700, background: T.cream, color: T.black, fontFamily: 'inherit' }}
              />
              <div style={{ fontSize: 10, color: T.muted, marginTop: 4 }}>
                Bisa pilih dari {settings.doc_upload_deadline_days || 3} hari yang lalu sampai kemarin
              </div>
            </div>

            {/* Note */}
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: T.muted, marginBottom: 6 }}>Keterangan Sakit</div>
              <textarea value={backdateNote} onChange={e => setBackdateNote(e.target.value)} rows={2}
                placeholder="Contoh: demam, flu, sakit kepala..."
            ...
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: T.muted, marginBottom: 6, display: 'flex', alignItems: 'center', gap: 6 }}>
                Upload Surat Dokter / Resep Obat
                <span style={{ fontSize: 9, background: T.purpleLight, color: T.purple, padding: '2px 7px', borderRadius: 20, fontWeight: 700 }}>SANGAT DIANJURKAN</span>
              </div>
              <label style={{ display: 'block', border: `1.5px dashed ${backdateDoc ? T.green : T.border}`, borderRadius: 12, padding: '16px', textAlign: 'center', cursor: 'pointer', background: backdateDoc ? T.greenLight : T.cream }}>
                <input type="file" accept="image/*,.pdf" style={{ display: 'none' }} onChange={e => setBackdateDoc(e.target.files[0])} />
                {backdateDoc
                  ? <div style={{ fontSize: 12, fontWeight: 700, color: T.green }}>✓ {backdateDoc.name}</div>
                  : <><div style={{ fontSize: 22, marginBottom: 4 }}>📎</div><div style={{ fontSize: 12, color: T.muted }}>Tap untuk upload foto atau PDF</div></>
                }
              </label>
              {!backdateDoc && (
                <div style={{ fontSize: 10, color: T.muted, marginTop: 4, lineHeight: 1.5 }}>
                  Tanpa dokumen → status akan menunggu verifikasi owner dan bisa dikunci sebagai CTB.
                </div>
              )}
            </div>
            ...
            {/* Error */}
            {backdateError && (
              <div style={{ background: T.dngBg, borderRadius: 10, padding: '10px 12px', fontSize: 12, color: T.dng, fontWeight: 600, marginBottom: 12 }}>
                ⚠ {backdateError}
              </div>
            )}
            ...
            {/* Buttons */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              <button onClick={() => { setBackdateModal(false); setBackdateDate(''); setBackdateNote(''); setBackdateDoc(null); setBackdateError(null) }}
              ...
              <button onClick={submitBackdate} disabled={backdateSubmitting || !backdateDate}
                style={{ padding: '13px', background: !backdateDate ? '#ccc' : T.black, color: T.white, border: 'none', borderRadius: 14, fontSize: 13, fontWeight: 800, cursor: (!backdateDate || backdateSubmitting) ? 'not-allowed' : 'pointer', fontFamily: 'inherit', opacity: backdateSubmitting ? .6 : 1 }}>
                {backdateSubmitting ? 'Menyimpan...' : '✓ Simpan'}
              </button>
            </div>
          </div>
        </div>
