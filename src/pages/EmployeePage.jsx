        ...
        <div style={{ marginBottom: 10 }}>
          ...
          {(() => {
            ...
            return (
              <>
                {isWeekend && (
                  <div style={{ background: C.wrnBg, border: `.5px solid ${C.wrnBd}`, borderRadius: 9, padding: '8px 12px', fontSize: 11, color: C.wrn, marginBottom: 8, lineHeight: 1.5 }}>
                    ⚠ Hari ini <strong>{dayName}</strong> (weekend). Day Off tidak tersedia.<br/>
                    Tidak hadir tanpa keterangan = <strong>Cuti Tidak Berbayar</strong>.
                  </div>
                )}
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 7 }}>
                  {[
                    { key: 'sakit', label: '🏥 Sakit', bg: C.infBg, color: C.inf, bd: C.infBd, available: true },
                    { key: 'cuti', label: '📅 Cuti', bg: C.okBg, color: C.ok, bd: C.okBd, available: true },
                    { key: 'day_off', label: '🌴 Day Off', bg: '#E8F5E9', color: '#2E7D32', bd: '#A5D6A7', available: !isWeekend },
                    { key: 'ctb', label: '◈ CTB', bg: C.purBg, color: C.pur, bd: C.purBd, available: true },
                  ].map(b => {
                    const disabled = !!todayRecord || !b.available
                    return (
                      <button key={b.key} onClick={() => !disabled && setModal(b.key)}
                        style={{ padding: '9px 6px', borderRadius: 9, border: `.5px solid ${disabled ? '#ddd' : b.bd}`, fontSize: 11, fontWeight: 500,
                          background: disabled ? '#f5f5f5' : b.bg, color: disabled ? '#bbb' : b.color,
                          cursor: disabled ? 'not-allowed' : 'pointer', fontFamily: 'inherit', textAlign: 'center',
                          opacity: !b.available ? 0.4 : 1 }}>
                        {b.label}
                        {!b.available && !todayRecord && <div style={{ fontSize: 9, opacity: .7 }}>Weekday only</div>}
                      </button>
                    )
                  })}
                </div>
          ...
          <div style={{ background: C.foam, borderRadius: '20px 20px 0 0', padding: '1.5rem', width: '100%', maxHeight: '85vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: '1rem' }}>
              <div style={{ width: 36, height: 36, borderRadius: 9,
                background: modal === 'sakit' ? C.infBg : modal === 'cuti' ? C.okBg : modal === 'day_off' ? '#E8F5E9' : C.purBg,
              ...
              <div>
                <div style={{ fontSize: 16, fontWeight: 500, color: C.esp, fontFamily: 'Georgia,serif' }}>
                  Catat {modal === 'sakit' ? 'Sakit' : modal === 'cuti' ? 'Cuti' : modal === 'day_off' ? 'Day Off' : 'Cuti Tidak Berbayar'}
                </div>
            ...
            {modal === 'day_off' && (
              <div style={{ background: '#E8F5E9', border: '.5px solid #A5D6A7', borderRadius: 9, padding: '8px 12px', marginBottom: '1rem', fontSize: 11, color: '#2E7D32', lineHeight: 1.5 }}>
                Day Off adalah hari libur mingguan Anda. Hanya tersedia di hari <strong>Senin–Jumat</strong>, maks. 1x per minggu. Menunggu persetujuan owner.
              </div>
            )}
