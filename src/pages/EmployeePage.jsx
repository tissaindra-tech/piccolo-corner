...
function Card({ children, style = {}, onClick }) {
  return (
    <div onClick={onClick} style={{ background: T.surface, borderRadius: 14, border: `.5px solid ${T.border}`, padding: '14px 16px', ...style }}>
      {children}
    </div>
  )
}
...
export default function EmployeePage() {
  ...
  async function fetchAll() {
    ...
    const { data: hist } = await supabase.from('attendance').select('*').eq('employee_id', user.id).order('date', { ascending: false }).limit(14)
    setHistory(hist || [])
  ...
  async function confirmAndSubmit() {
    ...
    if (pendingAction === 'checkin') {
      ...
      const openMs = oh * 60 + om, nowMs = now.getHours() * 60 + now.getMinutes()
      const isLate = nowMs > openMs + settings.late_tolerance_minutes
      const lateMinutes = isLate ? nowMs - openMs : 0
      const { error } = await supabase.from('attendance').upsert({
        employee_id: user.id, date: today, check_in: now.toISOString(), status: 'hadir',
      ...
      if (!error) showToast('ok', isLate ? `Check In ✓ — Terlambat ${lateMinutes} mnt` : 'Check In berhasil! ✓')
      else showToast('error', 'Gagal check in')
    } else {
      // Step 1: Update check_out dulu (paling penting)
      const { error: errOut } = await supabase.from('attendance')
        .update({ check_out: now.toISOString() })
        .eq('id', todayRecord.id)

      if (errOut) {
        showToast('error', `Gagal check out: ${errOut.message}`)
        setSubmitting(false)
        setScanStep('ready')
        return
      }

      // Step 2: Update optional fields (GPS + selfie) — tidak blocking
      const existingNote = todayRecord?.note || ''
      const extras = {}
      if (gpsInfo?.lat) { extras.gps_lat_out = gpsInfo.lat; extras.gps_lng_out = gpsInfo.lng; extras.gps_dist_out = gpsInfo.dist }
      if (selfieUrl) extras.note = existingNote ? `${existingNote}|selfie_out:${selfieUrl}` : `selfie_out:${selfieUrl}`
      if (Object.keys(extras).length > 0) {
        await supabase.from('attendance').update(extras).eq('id', todayRecord.id)
      }

      showToast('ok', 'Check Out berhasil! ✓')
    }
    setScanStep('done'); setSubmitting(false); fetchAll()
  }
  ...
  const canCheckOut = todayRecord?.check_in && !todayRecord?.check_out  // GPS tidak wajib untuk clock out
  const todaySt = todayRecord ? (statusConfig[todayRecord.status] || {}) : null

  const inputSt = { width: '100%', padding: '10px 12px', border: `.5px solid ${T.border}`, borderRadius: 10, fontSize: 13, background: T.bg, color: T.black, fontFamily: 'inherit' }
  ...
  return (
    <div style={{ minHeight: '100vh', background: T.bg, fontFamily: "'Inter', -apple-system, sans-serif", display: 'flex', flexDirection: 'column' }}>
      ...
      {/* ── CONTENT ── */}
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px', paddingBottom: 80, display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {tab === 'home' && <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <Card style={{ padding: '12px 14px' }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: T.muted, textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 4 }}>Sisa Cuti</div>
              <div style={{ fontSize: 26, fontWeight: 800, color: leaveBalance <= 3 ? T.red : T.green }}>{leaveBalance}</div>
              <div style={{ fontSize: 10, color: T.muted }}>hari tersisa</div>
            </Card>
            <Card style={{ padding: '12px 14px' }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: T.muted, textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 4 }}>Minggu Ini</div>
              <div style={{ fontSize: 26, fontWeight: 800, color: T.blue }}>{history.filter(h => { const diff = Math.floor((now - new Date(h.date))/86400000); return diff < 7 && h.status === 'hadir' }).length}</div>
              <div style={{ fontSize: 10, color: T.muted }}>hari hadir</div>
            </Card>
          </div>
          ...
          {gpsState==='blocked' && (
            <Card style={{ textAlign: 'center', padding: '20px 16px' }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 800, color: T.black, marginBottom: 4 }}>Di luar area cafe</div>
              <div style={{ fontSize: 12, color: T.muted, lineHeight: 1.6, marginBottom: 10 }}>
                Check In hanya bisa dari dalam cafe. Tapi <strong>Check Out bisa dari mana saja</strong>!
              </div>
              {gpsInfo?.dist && (
          ...
          {/* Scan / Camera — Check In butuh GPS, Check Out tidak */}
          {(gpsState === 'ok' || canCheckOut) && (
            <Card>
              <SLabel>Verifikasi Wajah</SLabel>
              {scanStep === 'ready' && (
                <>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}>
                    <div style={{ width: 80, height: 80, borderRadius: '50%', background: T.bg, border: `2px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 36 }}>🧑</div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    ...
                    <button disabled={!canCheckOut} onClick={() => startCamera('checkout')}
                      style={{ padding: '12px', background: canCheckOut ? T.orange : T.grayLight,
              ...
              {scanStep === 'camera' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <div style={{ background: T.blueLight, borderRadius: 9, padding: '7px 12px', fontSize: 11, color: T.blue, fontWeight: 600, textAlign: 'center' }}>
                    📷 {pendingAction === 'checkin' ? 'Selfie untuk Check In' : 'Selfie untuk Check Out'}
                  </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button onClick={confirmAndSubmit} disabled={submitting} style={{ flex: 2, padding: '11px', background: T.black, color: '#fff', border: 'none', borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', opacity: submitting ? .6 : 1 }}>
                      {submitting ? 'Menyimpan...' : pendingAction === 'checkin' ? '✓ Check In' : '✓ Check Out'}
                    </button>
                  </div>
                </div>
              )}
